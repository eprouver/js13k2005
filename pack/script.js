...
const css = `#sh,.screen{height:100vh}#start .center,#start-cat,#timer,canvas{position:absolute}#game,.screen{position:relative;width:100vw}#angler,#game,.screen{width:100vw}#angler,body{display:flex}.cell,.holder{transition:1s}*,.cell *{pointer-events:none}#range,.om,.reflect .c1,body{overflow:hidden}#mho,#start .center,.holder{text-align:center}#mho,#timer{font-weight:700}#canvas-2,.cho{z-index:2}*{transform-style:preserve-3d;perspective:5000px;transform-origin:50% 50%}body{--boardRot:50deg;--bopacity:0.25;--baseTransition:0.2s ease all;--ease1:linear(
    0,
    -0.004 4.9%,
    -0.02 9.4%,
    -0.124 26.4%,
...
const GAME_CONFIG = {
  ...
  STAR_SPAWN_INTERVAL: 800, // How often stars spawn (1 second)
};
...
float d(vec3 p){p.z+=s*2.0;p.x+=sin(p.z*0.5)*2.0;return c(p,vec3(0.0,0.0,1.5));}
...
void mainImage(out vec4 f,in vec2 c){vec3 o=vec3(0.0,0.0,-3.0);vec3 d=normalize(vec3((-1.0+2.0*c.xy/iResolution.xy)*vec2(iResolution.x/iResolution.y,1.0),1.0));o.x+=cos(s)*2.5;d.xy=r(d.xy,s*0.05+cos(s*0.05));d.x+=sin(s-3.14*0.5)*0.5;float t=0.0;vec4 l=vec4(0.07,0.1,0.15,0.0);for(int i=0;i<120;i++){vec4 v=d4(o+d*t);v.w*=0.35;v.xyz*=v.w;l=l+v*(1.0-l.w);t+=0.1;}l.xyz/=l.w;l=sqrt(l);f=vec4(l.xyz,1.0)*0.5;}`
  )
);
sources.push(
  initScene(
